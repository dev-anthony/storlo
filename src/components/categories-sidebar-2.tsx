...
export function CategoriesSidebar({ selectedCategory, onCategorySelect }: CategoriesSidebarProps) {
  return (
    <div className="lg:w-64 lg:shrink-0">
      <div className="lg:sticky lg:top-4">
        <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 scrollbar-hide">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className={`shrink-0 w-10 h-10 rounded-lg flex items-center justify-center transition-colors
                    ${isSelected ? 'bg-blue-100' : 'bg-gradient-to-br from-gray-100 to-gray-200'}`}>
                    <Icon className={`w-5 h-5 ${isSelected ? 'text-blue-600' : 'text-primary'}`} />
                  </div>
                  <div className="flex-1 min-w-0 text-left">
                    <p className={`font-medium text-sm ${isSelected ? 'text-blue-600' : 'text-gray-900 group-hover:text-primary'}`}>
                      {cat.name}
                    </p>
                    <p className="text-xs text-gray-500">{cat.items}</p>
                  </div>
                </div>
                <svg className={`w-5 h-5 shrink-0 ${isSelected ? 'text-blue-500' : 'text-gray-400'}`}
                  fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </button>
