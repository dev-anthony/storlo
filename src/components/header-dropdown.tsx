...
import { useRouter } from 'next/navigation';
import { ShoppingCart, User, Wallet, Settings, LogOut, Eye, EyeOff, Plus, Tag } from 'lucide-react';
import { useState } from 'react';
...
export function HeaderDropdown({ onClose, showSell = false }: HeaderDropdownProps) {
  const router = useRouter();
  const [showBalance, setShowBalance] = useState(false);

  const navItems = [
    ...(showSell ? [{ label: 'Sell', href: '/sell', icon: Tag }] : []),
    { label: 'Profile',  href: '/profile',               icon: User },
    { label: 'Orders',   href: '/orders',                icon: ShoppingCart },
    { label: 'Settings', href: '/settings',              icon: Settings },
    { label: 'Logout',   href: '/authentication/signin', icon: LogOut },
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors
                ...
                ${isSell   ? 'text-blue-600 font-semibold hover:bg-blue-50' : ''}
                ${!isLogout && !isSell ? 'text-gray-700 hover:bg-gray-50'  : ''}
                ${index !== navItems.length - 1 ? 'border-b border-gray-100' : ''}
              `}
            >
              <Icon className="h-4 w-4 flex-shrink-0" />
